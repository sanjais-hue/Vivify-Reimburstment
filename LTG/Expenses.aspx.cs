    ...
    values(@ServiceId, @TransportType, @Amount, @ExpenseType, @Date, 
        ...
        protected void btnImportExcel_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (var package = new OfficeOpenXml.ExcelPackage(ms))
                {
                    ...
                    int colRemarks     = GetColIndex(colMap, new[] { "remarks" }, 19);
                    ...
                    for (int r = dataStartRow; r <= totalRows; r++)
                    {
                        ...
                        string firstCellRaw = string.Concat(
                            ...
                            ws.Cells[r, 2].Text ?? "",
                            ws.Cells[r, 3].Text ?? "",
                            ws.Cells[r, colParticulars > 0 ? colParticulars : 6].Text ?? "").Trim().ToLower();
                        if (firstCellRaw.Contains("total") || firstCellRaw.Contains("grand"))
                        {
                            // Try to read from the Total column directly
                            decimal rowTotal = 0;
                            if (colTotal > 0)
                            {
                                TryParsePositiveDecimal((ws.Cells[r, colTotal].Text ?? "").Trim(), out rowTotal);
                            }
                            // Fallback: sum all numeric value cells in this row
                            if (rowTotal == 0)
                            {
                                for (int c2 = colConveyance; c2 <= Math.Min(colConveyance + 10, totalCols); c2++)
                                {
                                    string cv2 = (ws.Cells[r, c2].Text ?? "").Trim();
                                    if (TryParsePositiveDecimal(cv2, out decimal cv2Dec)) rowTotal += cv2Dec;
                                }
                            }
                            if (rowTotal > 0) Session["ExcelGrandTotal"] = rowTotal;
                            skipCount++;
                            continue;
                        }
                        ...
                        bool hasMisc       = TryParsePositiveDecimal(rawMisc,       out decimal miscAmt);
                        bool hasOthers     = TryParsePositiveDecimal(rawOthers,     out decimal othersAmt);

                        if (!hasConveyance && !hasLodging && !hasFooding && !hasMisc && !hasOthers) { skipCount++; continue; }
                        ...
                        if (hasMisc)
                        {
                            ...
                            drM["RowId"]         = ++rowId;
                            drM["ExpenseType"]   = expType;
                            drM["Category"]      = "Miscellaneous";
                            drM["Date"]          = isoDate;
                            drM["FromTime"]      = fromTime;
                            drM["ToTime"]        = toTime;
                            drM["Particulars"]   = rawParticulars;
                            drM["Distance"]      = "";
                            drM["TransportType"] = "";
                            drM["Amount"]        = miscAmt.ToString("0.##");
                            drM["SONo"]          = rawSONo;
                            drM["SMONo"]         = rawSMONo;
                            drM["RefNo"]         = rawRefNo;
                            drM["Remarks"]       = rawRemarks;
                            dt.Rows.Add(drM);
                        }
                        if (hasOthers)
                        {
                            ...
                            drO["RowId"]         = ++rowId;
                            drO["ExpenseType"]   = expType;
                            drO["Category"]      = "Others";
                            drO["Date"]          = isoDate;
                            drO["FromTime"]      = fromTime;
                            drO["ToTime"]        = toTime;
                            drO["Particulars"]   = rawParticulars;
                            drO["Distance"]      = "";
                            drO["TransportType"] = "";
                            drO["Amount"]        = othersAmt.ToString("0.##");
                            drO["SONo"]          = rawSONo;
                            drO["SMONo"]         = rawSMONo;
                            drO["RefNo"]         = rawRefNo;
                            drO["Remarks"]       = rawRemarks;
                            dt.Rows.Add(drO);
                        }
                    }
                }
        ...
        protected void gvExcelPreview_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "SaveRow")
            {
                ...
                try
                {
                    InsertExcelRowToDb(row, serviceId);

                    // Remove saved row from preview and refresh grid IN-PLACE (no redirect)
                    dt.Rows.Remove(row);
                    Session["ExcelPreviewData"] = dt;
                    BindExcelPreview(dt);
                    if (dt.Rows.Count == 0) pnlExcelPreview.Visible = false;

                    // Refresh the totals grids without a full redirect
                    DisplayExpenses(serviceId);

                    lblError.Text = "Row saved successfully.";
                    lblError.ForeColor = System.Drawing.Color.Green;
                    lblError.Visible = true;
                }
                catch (Exception ex)
                {
                    lblError.Text = "Error saving row: " + ex.Message;
                    lblError.ForeColor = System.Drawing.Color.Red;
                    lblError.Visible = true;
                }
                return;
            ...
            FillFormFromExcelRow(row2);

            // Remove from preview so user knows it's been loaded into the form
            dt.Rows.Remove(row2);
            Session["ExcelPreviewData"] = dt;
            BindExcelPreview(dt);
            if (dt.Rows.Count == 0) pnlExcelPreview.Visible = false;
        ...
        private void InsertExcelRowToDb(DataRow row, int serviceId)
        {
            ...
            string category  = row["Category"].ToString();      // Conveyance / Food / Lodging / Miscellaneous / Others
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                ...
                using (SqlTransaction transaction = con.BeginTransaction())
                {
                    try
                    {
                        if (category == "Conveyance")
                        {
                            // Normalise transport type for DB
                            string dbTransport = transport;
                            if (dbTransport == "Cab/Bus") dbTransport = "Cab/Bus";
                            InsertConveyanceExpense(con, transaction, serviceId, dbTransport, amount, date,
                                fromTime, toTime, particulars, remarks, null, expType, distance, smoNo, refNo, soNo);
                        }
                        else if (category == "Food")
                        {
                            TimeSpan? ft = string.IsNullOrEmpty(fromTime) ? (TimeSpan?)null : TimeSpan.Parse(fromTime);
                            TimeSpan? tt = string.IsNullOrEmpty(toTime)   ? (TimeSpan?)null : TimeSpan.Parse(toTime);
                            InsertFoodExpense(con, transaction, serviceId, expType, amount, date, DBNull.Value,
                                ft, tt, particulars, remarks, smoNo, refNo, soNo);
                        }
                        else if (category == "Lodging")
                        {
                            InsertLodgingExpenseSimple(con, transaction, serviceId, amount, date,
                                fromTime, toTime, particulars, remarks, smoNo, refNo, soNo);
                        }
                        else if (category == "Miscellaneous")
                        {
                            TimeSpan ft2 = string.IsNullOrEmpty(fromTime) ? TimeSpan.Zero : TimeSpan.Parse(fromTime);
                            TimeSpan tt2 = string.IsNullOrEmpty(toTime)   ? TimeSpan.Zero : TimeSpan.Parse(toTime);
                            InsertMiscellaneousExpense(con, transaction, serviceId,
                                particulars, amount, date, null, expType, ft2, tt2,
                                particulars, remarks, smoNo, refNo, soNo);
                        }
                        else if (category == "Others")
                        {
                            InsertOthersExpenseSimple(con, transaction, serviceId, amount, date,
                                fromTime, toTime, particulars, remarks, smoNo, refNo, soNo, expType);
                        }
                        transaction.Commit();
                    }
                    catch
        ...
        {
            if (string.IsNullOrEmpty(amount) || string.IsNullOrEmpty(date)) return;

            HttpCookie firstNameCookie = Request.Cookies["FirstName"];
            string firstName = firstNameCookie?.Value;

            string sql = @"INSERT INTO Lodging
                (ServiceId, ExpenseType, Date, Amount, FromTime, ToTime, Particulars, Remarks,
                 Image, ServiceReport, ApprovalMail, SmoNo, RefNo, SoNo, CreatedDate, CreatedBy)
                VALUES
                (@ServiceId, 'Tour', @Date, @Amount, @FromTime, @ToTime, @Particulars, @Remarks,
                 NULL, NULL, NULL, @SmoNo, @RefNo, @SoNo, @CreatedDate, @CreatedBy)";
            ...
            using (SqlCommand cmd = new SqlCommand(sql, con, transaction))
            {
                ...
                cmd.Parameters.AddWithValue("@CreatedBy", string.IsNullOrEmpty(firstName) ? (object)DBNull.Value : firstName);
                cmd.ExecuteNonQuery();
            }
        }
            ...
            string particulars, string remarks, string smoNo, string refNo, string soNo, string expenseType)
        {
            ...
            string sql = @"INSERT INTO Others
                (ServiceId, ExpenseType, Date, Amount, FromTime, ToTime, Particulars, Remarks,
                 Image, SmoNo, RefNo, SoNo, ServiceReport, ApprovalMail, CreatedDate, CreatedBy)
                VALUES
                (@ServiceId, @ExpenseType, @Date, @Amount, @FromTime, @ToTime, @Particulars, @Remarks,
                 NULL, @SmoNo, @RefNo, @SoNo, NULL, NULL, @CreatedDate, @CreatedBy)";

            using (SqlCommand cmd = new SqlCommand(sql, con, transaction))
            {
                cmd.Parameters.AddWithValue("@ServiceId",   serviceId);
                cmd.Parameters.AddWithValue("@ExpenseType", string.IsNullOrEmpty(expenseType) ? "Local" : expenseType);
                cmd.Parameters.AddWithValue("@Date",   DateTime.Parse(date));
                cmd.Parameters.AddWithValue("@Amount", decimal.Parse(amount));
                cmd.Parameters.AddWithValue("@FromTime", string.IsNullOrEmpty(fromTime) ? (object)DBNull.Value : TimeSpan.Parse(fromTime));
                cmd.Parameters.AddWithValue("@ToTime",   string.IsNullOrEmpty(toTime)   ? (object)DBNull.Value : TimeSpan.Parse(toTime));
